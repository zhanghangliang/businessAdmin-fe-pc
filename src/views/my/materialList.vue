        ...
        <template v-if="historyData.length != 0">
          ...
          <van-collapse v-model="activeNames" class="record-div">
            ...
            >
                ...
                <div class="record-list">
                  <div class="left">
                    <span
                      ...
                      <van-tag type="success" v-if="item.changeAudit === 1"
                        >已转预审</van-tag
                      >
                    </span>
                    <p class="record-list-time">{{ item.createTime }}</p>
                  </div>
                  <!-- <span class="right" :style="{color: item.status==2?'#07c160':item.status==3?'#f40':'#ACADB1'}">{{item.status==2?'已通过':item.status==3?'未通过':'审核中'}}</span> -->
                  <span @click.stop="toDetail(item)" style="color: #1989fa"
                    >详情</span
                  >
                </div>
              </template>
              <!-- <div class="steps-div">
                            <p>
                              <span@click.stop="toDetail(item)">详情</span>
                            </p>
                           <van-steps direction="vertical" :active="item.status==1?0:1"  active-color="#3D65E6">
                                <van-step>
                                    <h4>审核中</h4>
                                    <p>{{item.createTime}}</p>
                                </van-step>
                                <van-step v-if="item.status==2">
                                    <h4>审核通过</h4>
                                    <p>{{item.updateTime}}</p>
                                </van-step>
                                <van-step v-if="item.status==3">
                                    <h4>审核未通过</h4>
                                    <p>{{item.updateTime}}</p>
                                    <div class="noPass-style">原因：{{item.reason}}</div>
                                </van-step>
                            </van-steps>
                            <div class="bottom-btn" v-if="item.status==2">
