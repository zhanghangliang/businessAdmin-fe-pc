...
    }, false);
  ...
  methods: {
      getList(){
        ...
        httpAction('/Wx/unoperationhistory', temp).then(response => {
            if (response.code == 0) {
      ...
      toDetail(data){
        ...
        temp.idsArr.push([data.matterId])

        this.ids = []
        if(data.operationRecord.length>0){
           this.getIds(data.operationRecord)
        }
        temp.idsArr = [...temp.idsArr,...this.ids]  
        temp.lastId = this.lastId
        temp.prevId = this.upoeration
        console.log(temp.idsArr)
        
        temp.isHistory = true  
        data.isHistory = true  
        this.$router.push({
            path: '/serverDetail', // /historyGoOn
            query: data
        })
      },
      getIds(data){
        data.forEach((item)=>{
            this.ids.push([item.matterId])
            if(item.operationRecord.length>0){
                this.getIds(item.operationRecord)
            }else{
                this.lastId = item.id
                this.upoeration = item.upoeration
            }
        })
