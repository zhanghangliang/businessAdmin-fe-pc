<template>
  <div>
    <div class="all" style="height:100vh">
      <div class="top-text">
        <p class="all-title">{{ tableData.subject ? tableData.subject.name : '' }}</p>
        <span>{{ tableData.subject ? tableData.subject.description : '' }}</span>
        <p v-if="reason" style="color: #ff4949;">未通过原因：{{ reason }}</p>
      </div>
      <ul class="con-list">
        ...
        <li>
          <template>
            <el-table ref="table" :data="tableData.detailList" border style="width: 100%">
              ...
              <el-table-column label="操作" width="80" align="center" v-if="selectData.isView == 1">
                <template slot-scope="scope">
                  <van-uploader
      ...
      <!-- <p class="viewFiles-style" @click="viewFiles">批量文件</p> -->
      <div class="bottom-btn">
        ...
        <van-uploader
          ...
          "
          v-if="selectData.isView == 1"
        >
          <el-button type="primary" size="small">上传</el-button>
        </van-uploader>
        <el-button type="primary" size="small" @click="backPage">{{ selectData.isView == 1 ? '提交审核' : '返回' }}</el-button>
      </div>
    </div>
    <!-- 查看详细 -->
      ...
      <div class="mobile-popup-con" v-if="popupShow">
        ...
        <div class="description">
          <ul v-if="files && files.length > 0">
            <li v-for="(item, index) in files" :key="index" @click="newFile(index)">
              <span>{{ index + 1 }}.{{ item.fileName }}</span>
              <!-- <i v-if="selectData.isView == 1" class="el-icon-delete" @click.stop="filesDel(item, index)" style="padding: 4px;"></i> -->
              <i class="i-down-btn" @click.stop="newFile(index)">查看</i>
              <i class="i-down-btn" @click.stop="newFile(index)">下载</i>
            </li>
            </li>
          </ul>
          <p v-else>暂无数据</p>
...
export default {
  ...
  methods: {
        ...
        .then(response => {
          if (response.code == 0) {
            ...
            if (response.data.status === 4) {
              this.reason = response.data.wxOperationRecordAuditList.filter(item => item.status === 4).reason;
            }
    ...
    backPage() {
      if (this.selectData.isView == 1 && this.$route.query.routeName !== '/materialList') {
        let toastItem = this.Toast.loading({
          duration: 0, // 持续展示 toast
          forbidClick: true,
          message: '提交中...'
        ...
        this.$router.replace({
          path: this.$route.query.routeName || '/record'
        });
      }
      // this.$router.go(-1);
    },
