<template>
  <div>
    <div ref="scrollDiv" class="all" id="all">
      <div ref="scrollItem" class="con-item-div answer" style="padding-top: 10px;">
        ...
        <ul v-for="(allItem,allItemIndex) in allData" :key="allItemIndex" class="serve-list last-span-none">
          <li class="firstNone">
            <span>{{ allItem.situation.matterName }}</span>
            <span style="display: none;" />

            <template v-if="allItem.sonSituation&&allItem.sonSituation.length>0">
              ...
              <template v-if="allItem.situation.strategy==1">
                <van-radio-group v-model="allDataId[allItemIndex]">
                  ...
                  >
                    <template #default>
                      {{ item.name }} <span class="warning-info" v-if="(allDataId[allItemIndex] == item.id)&&item.forceRemind">{{item.remindContent}}</span>
                      <span class="detail-info" @click.stop="showDetail(item.description)">详情</span>
                    </template>
                  </van-radio>
                </van-radio-group>
              </template>
              <template v-else-if="allItem.situation.strategy==2">
                <van-checkbox-group v-model="allDataId[allItemIndex]">
                  <van-checkbox
                    ...
                    :class="{isCkecked:allDataId[allItemIndex] == item.id}"
                    shape="square"
                    @click="checkboxClick(item,allItemIndex,0,allItem.situation.lev)"
                  >
                    <template #default>
                      {{ item.name }} <span class="warning-info"  v-if="(allDataId[allItemIndex] == item.id)&&item.forceRemind">{{item.remindContent}}</span>
                      <span class="detail-info" @click.stop="showDetail(item.description)">详情</span>
    ...
    <div class="bottom-div">
      <div class="btn-div" style="text-align: center;">
        <button class="left" style="width: calc(100% - 20px);" @click="getMaterial">材料清单</button>
      </div>
    </div>
    <!-- 查看详细 -->
...
export default {
  ...
  methods: {
    ...
    getLevelData(data, liItem) {
      ...
      httpAction('/Wxoperation/findNextQuestion', { body: {subjectId:this.liData.id}}).then(res => {
        if (res.code == 0) {
    ...
    getMaterial() {
      ...
      if (flag) {
        ...
        let temp = {
            ...
            // }
        httpAction('/Wxoperation/preliminaryExaminationOperation', {body:temp}).then(response => {
           if (response.code == 0) {
               this.$router.push({
                  path: '/preliminaryTable',
                  query: {
    ...
    radioClick(item, index, type) {
      let temp = {
        lastQaRelationShipId:this.allData[index].situation.lastQaRelationShipId,
        opinionId:item.id
      }
      ...
      this.getNextData(temp, (res) => {
        ...
        for (let i = this.allData.length - 1; i >= 0; i--) {
          ...
          if ((this.allData[i].situation.topId.includes(temp.lastQaRelationShipId))&&(this.allData[i].situation.lev>curLev)) {
    ...
    checkboxClick(item, index, type, lev) {
      for (let i = this.allData.length - 1; i >= 0; i--) {
        if ((this.allData[i].situation.topId == this.allData[index].situation.topId)&&(this.allData[i].situation.lev > lev)) {
    ...
    //  /Matter/queryDepositoryByParent
      ...
      httpAction('/Wxoperation/findNextQuestion', { body: temp}).then(res => {
        if (res.code == 0) {
